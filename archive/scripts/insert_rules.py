...
# 读取原文件
with open('C:/Users/10952/.openclaw/workspace/MEMORY.md', 'r', encoding='utf-8', errors='ignore') as f:
    content = f.read()
...
---
"""
    new_content = content.replace(insert_marker, new_rules + "\n" + insert_marker)
    
    # 写回文件
    with open('C:/Users/10952/.openclaw/workspace/MEMORY.md', 'w', encoding='utf-8') as f:
        f.write(new_content)
    
    print("✅ 规则 5 和规则 6 已成功写入 MEMORY.md！")
    print("📄 验证：规则 5 存在 =", "规则 5" in new_content)
    print("📄 验证：规则 6 存在 =", "规则 6" in new_content)
else:
    print("❌ 未找到任何插入标记")
    print("文件前 500 字符:")
    print(content[:500])
